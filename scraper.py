...
class Scraper:
    ...
    def get_drama_list(self, rating_type='imdb'):
        soup = self.fetch_page()
        lst = soup.find('ul', class_='ipc-metadata-list ipc-metadata-list--dividers-between sc-d24d5d37-0 hDHQeM detailed-list-view ipc-metadata-list--base')
        dramas = []
        for item in lst.find_all('li', class_='ipc-metadata-list-summary-item'):
            ...
            drama_object['rating_score'] = rating.get_text(strip=True)
            ...
            drama_object['short_description'] = description.get_text(strip=True)
